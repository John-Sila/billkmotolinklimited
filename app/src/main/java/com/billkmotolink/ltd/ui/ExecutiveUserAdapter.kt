...
import com.billkmotolink.ltd.databinding.ExecutiveUserItemBinding
                ...
                "HR" -> {
                    ...
                    binding.summaryTextView.text =
                        when (user.isClockedIn) {
                            true -> {
                                "${user.userName?.substringBefore(" ")} is clocked in."
                            }

                            false -> {
                                "$formattedDate, ${user.userName?.substringBefore(" ")}'s clockout recorded a net income of ${
                                    formatIncome(
                                        user.netIncome ?: 0.0
                                    )
                                }."
