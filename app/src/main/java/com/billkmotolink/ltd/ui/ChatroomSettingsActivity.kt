...
import com.billkmotolink.ltd.databinding.ActivityChatroomSettingsBinding
        ...
        val approvedAdapter = UsernameAdapter(
            ...
            onRemove = { uid ->
                rtdb.child("chatrooms").child(chatroomId)
                    .child("approvedParticipants").get().addOnSuccessListener {
                        val list =
                            it.getValue(object : GenericTypeIndicator<MutableList<String>>() {})
                                ?: mutableListOf()
                        list.remove(uid)
                        rtdb.child("chatrooms").child(chatroomId).child("approvedParticipants")
                            .setValue(list)
                    }
            },
            onApprove = {},
            ...
            onLeave = { uid ->
                ...
                chatRef.child("approvedParticipants").get().addOnSuccessListener { appSnap ->
                    val appList =
                        appSnap.getValue(object : GenericTypeIndicator<MutableList<String>>() {})
                            ?: mutableListOf()
        ...
        val pendingAdapter = UsernameAdapter(
            ...
            onApprove = { uid ->
                ...
                chatRef.child("pendingApprovals").get().addOnSuccessListener { pendSnap ->
                    chatRef.child("approvedParticipants").get().addOnSuccessListener { appSnap ->
                        val pendList = pendSnap.getValue(object :
                            GenericTypeIndicator<MutableList<String>>() {}) ?: mutableListOf()
                        val appList = appSnap.getValue(object :
                            GenericTypeIndicator<MutableList<String>>() {}) ?: mutableListOf()
                        pendList.remove(uid)
                        appList.add(uid)
            ...
            onDeny = { uid ->
                ...
                chatRef.child("pendingApprovals").get().addOnSuccessListener { snap ->
                    val list =
                        snap.getValue(object : GenericTypeIndicator<MutableList<String>>() {})
                            ?: mutableListOf()
                    list.remove(uid)
